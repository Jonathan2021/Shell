...
{
    char *grammar[20][20] = {{"SEMICOLON", ";", "\0"},
        {"OP_LOGIQUE", "&&", "||", ";;", "\0"},
        {"OP_IO", "<<", ">>", "<&", ">&", "<>", "<<-", "\0"},
        {"CLOBBER", ">|", "\0"}, {"IF", "if", "\0"}, {"FI", "fi", "\0"},
        {"THEN", "then", "\0"}, {"ELIF", "elif", "\0"}, {"ELSE", "else", "\0"},
...
struct Token *parse_path(struct Token *token, char **argv, long argc)
{
    ...
    int option_index = 0;
    optind = 0;
    while ((c = getopt_long(argc, argv, "c:vnat", long_options, &option_index))
           != -1)
    {
        if (c == 'c')
        {
